...
def fill_movie_details(movie):
    """
    Fetch a movie's full details from OMDb. Then, save it to the DB. If the movie already has a `full_record` this does nothingm so it's safe to call with any `movie`.
    """
...
def search_and_save(search):
    """
    perform a search for search_term against the API, but only if it hasn't been seatcged in the past 24 hours. Save each result to the local DB as a partial record.
    """
